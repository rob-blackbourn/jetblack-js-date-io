import { utilsTest, formats, TEST_TIMESTAMP } from './utils'
...
describe('DateTime calculations', () => {
  utilsTest('date', (date, utils) => {
    ...
    // native Date
    expect(
      utils.isEqual(date, utils.date(new Date(TEST_TIMESTAMP)))
    ).toBeTruthy()
    // parse already date-specific object
    expect(
      utils.isEqual(date, utils.date(utils.date(TEST_TIMESTAMP)))
    ).toBeTruthy()
    // parse null inputs
  ...
  utilsTest('isValid', (date, utils) => {
    ...
    const foo = utils.isValid('2018-42-30T11:60:00.000Z')
  ...
  utilsTest('addMonths', (date, utils, lib) => {
    expect(utils.format(utils.addMonths(date, 2), 'monthAndYear')).toBe(
      'December 2018'
    )
    expect(utils.format(utils.addMonths(date, -2), 'monthAndYear')).toBe(
      'August 2018'
  ...
  utilsTest('startOfDay', (date, utils, lib) => {
    expect(
      utils.formatByString(utils.startOfDay(date), formats.dateTime[lib])
    ).toBe('2018-10-30 00:00')
  })

  utilsTest('endOfDay', (date, utils, lib) => {
    expect(
      utils.formatByString(utils.endOfDay(date), formats.dateTime[lib])
    ).toBe('2018-10-30 23:59')
  })

  utilsTest('startOfYear', (date, utils, lib) => {
    expect(
      utils.formatByString(utils.startOfYear(date), formats.dateTime[lib])
    ).toBe('2018-01-01 00:00')
  })

  utilsTest('endOfYear', (date, utils, lib) => {
    expect(
      utils.formatByString(utils.endOfYear(date), formats.dateTime[lib])
    ).toBe('2018-12-31 23:59')
  })

  utilsTest('startOfMonth', (date, utils, lib) => {
    expect(
      utils.formatByString(utils.startOfMonth(date), formats.dateTime[lib])
    ).toBe('2018-10-01 00:00')
  })

  utilsTest('endOfMonth', (date, utils, lib) => {
    expect(
      utils.formatByString(utils.endOfMonth(date), formats.dateTime[lib])
    ).toBe('2018-10-31 23:59')
  })

  utilsTest('startOfWeek', (date, utils, lib) => {
    expect(
      utils.formatByString(utils.startOfWeek(date), formats.dateTime[lib])
    ).toBe(lib === 'JetblackDate' ? '2018-10-29 00:00' : '2018-10-28 00:00')
  })

  utilsTest('endOfWeek', (date, utils, lib) => {
    expect(
      utils.formatByString(utils.endOfWeek(date), formats.dateTime[lib])
    ).toBe(lib === 'JetblackDate' ? '2018-11-04 23:59' : '2018-11-03 23:59')
  })
  ...
  utilsTest('startOfWeekNonISO', (date, utils, lib) => {
    ...
    ).toBe(lib === 'JetblackDate' ? '2018-10-22 00:00' : '2018-10-28 00:00')
  })
  ...
  utilsTest('endOfWeekNonISO', (date, utils, lib) => {
    ...
    ).toBe(lib === 'JetblackDate' ? '2018-10-28 23:59' : '2018-11-03 23:59')
  })
  ...
  utilsTest('getNextMonth', (date, utils, lib) => {
    expect(
      utils.formatByString(utils.getNextMonth(date), formats.dateTime[lib])
    ).toBe('2018-11-30 11:44')
  })
  ...
  utilsTest('getDiff', (date, utils) => {
    expect(utils.getDiff(date, utils.date('2018-10-29T11:44:00.000Z'))).toBe(
      86400000
    )
    expect(utils.getDiff(date, utils.date('2018-10-31T11:44:00.000Z'))).toBe(
      -86400000
    )
  ...
  utilsTest('getDiff with units', (date, utils) => {
    expect(
      utils.getDiff(date, utils.date('2017-09-29T11:44:00.000Z'), 'years')
    ).toBe(1)
    expect(
      utils.getDiff(date, utils.date('2018-08-29T11:44:00.000Z'), 'months')
    ).toBe(2)
    expect(
      utils.getDiff(date, utils.date('2018-05-29T11:44:00.000Z'), 'quarters')
    ).toBe(1)
    expect(
      utils.getDiff(date, utils.date('2018-09-29T11:44:00.000'), 'days')
    ).toBe(31)
    expect(
      utils.getDiff(date, utils.date('2018-09-29T11:44:00.000Z'), 'weeks')
    ).toBe(4)
    expect(
      utils.getDiff(date, utils.date('2018-09-29T11:44:00.000Z'), 'hours')
    ).toBe(744)

    expect(
      utils.getDiff(date, utils.date('2018-09-29T11:44:00.000Z'), 'minutes')
    ).toBe(44640)

    expect(
      utils.getDiff(date, utils.date('2018-10-30T10:44:00.000Z'), 'seconds')
    ).toBe(3600)

    expect(
      utils.getDiff(
        date,
        utils.date('2018-10-30T10:44:00.000Z'),
        'milliseconds'
      )
    ).toBe(3600000)
  })
  ...
  utilsTest('mergeDateAndTime', (date, utils, lib) => {
    ...
    expect(utils.toJsDate(mergedDate).toISOString()).toBe(
      '2018-10-30T14:15:16.000Z'
    )
  })
  ...
  utilsTest('isSameDay', (date, utils, lib) => {
    expect(
      utils.isSameDay(date, utils.date('2018-10-30T00:00:00.000Z'))
    ).toBeTruthy()
    expect(
      utils.isSameDay(date, utils.date('2019-10-30T00:00:00.000Z'))
    ).toBeFalsy()
  })

  utilsTest('isSameMonth', (date, utils, lib) => {
    expect(
      utils.isSameMonth(date, utils.date('2018-10-01T00:00:00.000Z'))
    ).toBeTruthy()
    expect(
      utils.isSameMonth(date, utils.date('2019-10-01T00:00:00.000Z'))
    ).toBeFalsy()
  })

  utilsTest('isSameYear', (date, utils, lib) => {
    expect(
      utils.isSameYear(date, utils.date('2018-10-01T00:00:00.000Z'))
    ).toBeTruthy()
    expect(
      utils.isSameYear(date, utils.date('2019-10-01T00:00:00.000Z'))
    ).toBeFalsy()
  })
  ...
  utilsTest(
    'getCurrentLocaleCode: returns default locale',
    (date, utils, lib) => {
  ...
  utilsTest(
    'isWithinRange: checks that dates isBetween 2 other dates',
    (date, utils) => {
      expect(
